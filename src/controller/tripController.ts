    ...
    try {
        ...
        if (bookingIds && bookingIds.length > 0) {
            const bookings = await prisma.booking.findMany({
                where: {
                    id: { in: bookingIds.map((id: string) => parseInt(id)) },
                    travellerId: user.id
                }
            });
        ...
        if (orderIds && orderIds.length > 0) {
            const orders = await prisma.order.findMany({
                where: {
                    id: { in: orderIds.map((id: string) => parseInt(id)) },
                    travellerId: user.id
                }
            });
        ...
        const trip = await prisma.trip.create({
            data: {
                ...
                ...(bookingIds && bookingIds.length > 0 && {
                    bookings: {
                        connect: bookingIds.map((id: string) => ({ id: parseInt(id) }))
                    }
                }),
                ...(orderIds && orderIds.length > 0 && {
                    orders: {
                        connect: orderIds.map((id: string) => ({ id: parseInt(id) }))
                    }
                })
            },
            include: {
    ...
    try {
        ...
        const updatedTrip = await prisma.trip.update({
            ...
            data: {
                ...
                ...(bookingIds && {
                    bookings: {
                        set: bookingIds.map((bid: string) => ({ id: parseInt(bid) }))
                    }
                }),
                ...(orderIds && {
                    orders: {
                        set: orderIds.map((oid: string) => ({ id: parseInt(oid) }))
                    }
                })
            },
            include: {
...
export const getTripRestaurants = async (req: Request, res: Response) => {
    try {
        const user = (req as any).user;
        const { id } = req.params;
        const tripId = Array.isArray(id) ? id[0] : id;
